...
export default function Elements ( props ) {
    ...
    const {
        ...
        color, icon, height, min, max, step, required,
    } = props
    ...
    return (
        ...
        <div>
            ...
            {
                element === 'image_edit' &&
                <div className={`relative group flex justify-center items-center select-none overflow-hidden bg-primary-light dark:bg-[#060818] p-1 m-auto border-2 border-primary/20 rounded-${type !== 'md' ? 'full' : 'md'} ${className || 'w-[9rem] h-[9rem]'} ${readOnly && 'layer'}`}>
                    <img 
                        src={src} 
                    ...
                    />
                    {
                        ...
                        <div onClick={() => ref.current?.click()} className='absolute top-0 left-0 w-full h-full cursor-pointer justify-center items-center flex-col bg-black/0 text-white flex group-hover:bg-black/50 transition-all duration-200 space-y-2'>
                            ...
                            <span className="text-[.9rem] hidden group-hover:block font-semibold">Update</span>
            ...
            }
            {
                ...
                <div className={`w-full ${className.includes('flex') ? 'lg:flex justify-center items-center gap-6 space-y-3 lg:space-y-0' : 'space-y-3'} ${className.replace('flex', '')}`}>
                    <label htmlFor={name}>
                        ...
                        { icon ? <span>{label || name}</span> : label || name }
                    </label>
                    <input id={name} type={type} required={required} value={value === undefined ? '' : value} onChange={(e) => onChange(e.target.value)} readOnly={readOnly} ref={ref} min='0' className={`form-input flex-1 ${readOnly ? 'cursor-default': ''}`} autoComplete="off"/>
                </div>
            }
            {
                element === 'textarea' &&
            ...
            }
            {
                ...
                <div className={`w-full flex justify-start items-center select-none ${className}`}>
                    ...
                    <label htmlFor={label} className="ltr:pl-3 rtl:pr-3 truncate cursor-pointer w-full flex-1 overflow-hidden text-gray-500 text-[.95rem] font-semibold tracking-wide">
            ...
            }
            {
                ...
                <div className={`w-full relative ${className.includes('flex') ? 'lg:flex justify-center items-center gap-6 space-y-3 lg:space-y-0' : 'space-y-3'} ${className.replace('flex', '')}`}>
                    {
                        visibility ?
                        <div className="ltr:right-[.5rem] rtl:left-[.5rem] absolute top-auto cursor-pointer rounded-full w-[1.7rem] h-[1.7rem] flex justify-center items-center bottom-[.35rem]" onClick={() => setVisibility(false)}>
                            <span className="material-symbols-outlined text-[1.2rem]">visibility</span>
                        </div> :
                        <div className="ltr:right-[.5rem] rtl:left-[.5rem] absolute top-auto cursor-pointer rounded-full w-[1.7rem] h-[1.7rem] flex justify-center items-center bottom-[.35rem]" onClick={() => setVisibility(true)}>
                            <span className="material-symbols-outlined text-[1.2rem]">visibility_off</span>
                        </div>
                    }
                    <label htmlFor={name}>
                        { icon && <span className='material-symbols-outlined'>{icon}</span> }
                    ...
                    <input id={name} required={required} type={visibility ? 'text' : 'password'} value={value || ''} onChange={(e) => onChange(e.target.value)} ref={ref} className={`form-input flex-1 ${readOnly ? 'cursor-default': ''}`} autoComplete="off"/>
                </div>
            }
            {
                element === 'upload' &&
            ...
            {
                element === 'save_button' &&
                <button type="button" onClick={onClick} className={`btn btn-success py-2.5 w-full select-none gap-2 shadow-none tracking-wide hover:opacity-[.8] ${className}`}>
                    <Icons icon='save'/>
            ...
            }
            {
                ...
                <button type="button" onClick={onClick} className={`btn btn-warning py-2.5 w-full select-none gap-2 shadow-none tracking-wide hover:opacity-[.8] ${className}`}>
                    <Icons icon='cancel'/>
            ...
            }
            {
                ...
                <button type="button" onClick={onClick} className={`btn btn-danger py-2.5 w-full select-none gap-2 shadow-none tracking-wide hover:opacity-[.8] ${className}`}>
                    <Icons icon='delete'/>
            ...
            }
            {
                ...
                <button type="button" onClick={onClick} className={`btn btn-primary py-2.5 w-full select-none gap-2 shadow-none tracking-wide hover:opacity-[.8] ${className}`}>
                    <Icons icon='upload'/>
                    <span>{config.text.upload_file}</span>
                </button>
            }
            ...
            {
                element === 'select_child' &&
                <div className={`dropdown ${className}`}>
                    <Dropdown
                        placement='reverse'
                        btnClassName='w-full'
                        button={
                            <div className='panel w-full flex justify-between items-center select-none py-2.5 px-4 text-[.9rem] text-gray-700 active:border-primary tracking-wide'>
                                <div className="flex justify-center items-center gap-1">
                                    <span>{label}</span>
                                    { value ? <span className="scale-[.85] -mt-[1px]">( {value} )</span> : ''}
                                </div>
                                <span className='material-symbols-outlined -mt-[1px] opacity-[.7]'>keyboard_arrow_down</span>
                            </div>
                        }
                    >
                        <div className='panel p-[1px] overflow-hidden' onClick={(e) => e.stopPropagation()}>

                            {/* {
                                label &&
                                <h1 className='flex justify-start items-center gap-2 p-5 pb-0 text-[.9rem] select-none font-semibold text-black tracking-wide'>
                                    <span>{label}</span>
                                </h1>
                            } */}

                            <div className='px-4 py-5 max-h-[21rem] overflow-y-auto small-bar'>
                                {children}
                            </div>
            ...
            }
            {
                ...
                <div className={`panel p-5 w-full h-full space-y-2.5 ${className}`}>
                    <h5 className="font-semibold text-[1.15rem] tracking-wide">{config.text[label || name]}</h5>
                    <p className='pb-1 leading-6 line-clamp-2 text-[.95rem] font-semibold tracking-wide opacity-[.95]' title={config.text[type]}>{config.text[type]}</p>
                    <label className="w-12 h-6 relative block">
                        ...
                        renderThumb={({ props }) => {
                            ...
                            return (
                                <div
                                    key={key}
                                    {...restProps}
                                    className="h-5 w-5 bg-primary-light border rounded-full focus:outline-none shadow-lg"
                                    style={{ ...props.style }}
                                />
                            )
                        }}
